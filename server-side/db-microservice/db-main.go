package main

import (
	"crypto/sha1"
	"database/sql"
	"encoding/hex"
	"encoding/json"
	"fmt"
	"log"
	"net/http"

	"github.com/dgrijalva/jwt-go"
	_ "github.com/go-sql-driver/mysql"
	"github.com/gorilla/mux"
	"github.com/rs/xid"

	"../data_structures/credentials"
	"../data_structures/request"
	"../utils"
)

var db *sql.DB
var tokens = make(map[string][]byte)

func authenticate(w http.ResponseWriter, r *http.Request) {
	body := utils.GetRequestBody(r)

	var credentials credentials.Credentials
	json.Unmarshal(body, &credentials)

	log.Println("Authenticate request for user", credentials.Username, "with password hash", credentials.PasswordHash)

	_, err := db.Exec("set @valid = Login(?, ?);", credentials.Username, credentials.PasswordHash)
	utils.CheckError(err)

	row := db.QueryRow("select @valid")

	valid := false
	row.Scan(&valid)

	if !valid {
		utils.WriteResponse(w, 500, []byte("Invalid password"))
		return
	}

	h := sha1.New()

	h.Write([]byte(credentials.Username))

	token := append([]byte(hex.EncodeToString(h.Sum(nil))), []byte(xid.New().String())...)

	tokens[credentials.Username] = token

	utils.WriteResponse(w, 200, token)
}

func getType(w http.ResponseWriter, r *http.Request) {
	body := utils.GetRequestBody(r)

	var username struct{ Username string }
	json.Unmarshal(body, &username)

	log.Println("Get type request for user", username.Username)

	_, err := db.Exec("set @type = GetType(?);", username.Username)
	utils.CheckError(err)

	row := db.QueryRow("select @type")

	t := ""
	row.Scan(&t)

	utils.WriteResponse(w, 200, []byte(t))
}

func validate(w http.ResponseWriter, r *http.Request) {
	body := utils.GetRequestBody(r)

	var request request.Request
	json.Unmarshal(body, &request)

	tokenString := string(request.Request)

	token, err := jwt.Parse(tokenString, func(token *jwt.Token) (interface{}, error) {
		if _, ok := token.Method.(*jwt.SigningMethodHMAC); !ok {
			return nil, fmt.Errorf("Unexpected signing method: %v", token.Header["alg"])
		}

		return tokens[request.Username], nil
	})

	if claims, ok := token.Claims.(jwt.MapClaims); ok && token.Valid {
		fmt.Println(claims["request"])

		// TODO Check permisions
	} else {
		log.Println(err)
	}
}

func main() {
	r := mux.NewRouter()

	db, _ = sql.Open("mysql", "root:root@tcp(mysql:3306)/university")
	defer db.Close()

	r.HandleFunc("/api/authenticate", authenticate).Methods("POST")
	r.HandleFunc("/api/type", getType).Methods("POST")
	r.HandleFunc("/api/validate", validate).Methods("POST")

	log.Fatal(http.ListenAndServe(":10000", r))
}
